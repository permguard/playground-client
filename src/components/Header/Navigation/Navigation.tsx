import { NavLink } from "./NavLink";

type INavigation = {
  name: string;
  sublinks?: {
    name: string;
    href: string;
    description: string;
    icon: React.FC;
  }[];
  href?: string;
  description?: string;
}[];

export const NAVIGATION: INavigation = [
  // {
  //   name: "Products",
  //   sublinks: [
  //     {
  //       name: "Community",
  //       href: "/community",
  //       description: "Collaborate and grow with fellow users",
  //       icon: () => (
  //         <svg
  //           xmlns="http://www.w3.org/2000/svg"
  //           width="24"
  //           height="24"
  //           viewBox="0 0 24 24"
  //         >
  //           <path
  //             fill="currentColor"
  //             d="M12 1C5.923 1 1 5.923 1 12c0 4.868 3.149 8.979 7.521 10.436c.55.096.756-.233.756-.522c0-.262-.013-1.128-.013-2.049c-2.764.509-3.479-.674-3.699-1.292c-.124-.317-.66-1.293-1.128-1.554c-.385-.206-.935-.715-.013-.729c.866-.014 1.485.797 1.691 1.128c.99 1.663 2.571 1.196 3.204.907c.096-.715.385-1.196.701-1.471c-2.447-.275-5.005-1.224-5.005-5.431c0-1.197.426-2.187 1.128-2.957c-.11-.275-.495-1.402.11-2.915c0 0 .92-.288 3.024 1.128c.88-.248 1.816-.372 2.75-.372c.936 0 1.87.124 2.75.372c2.104-1.43 3.025-1.128 3.025-1.128c.605 1.513.22 2.64.11 2.915c.702.77 1.128 1.747 1.128 2.957c0 4.22-2.571 5.156-5.019 5.43c.399.345.743 1.005.743 2.036c0 1.471-.014 2.654-.014 3.025c0 .288.206.632.756.522A11.02 11.02 0 0 0 23 12c0-6.077-4.922-11-11-11"
  //           />
  //         </svg>
  //       ),
  //     },
  //     {
  //       name: "Enterprise",
  //       href: "/enterprise",
  //       description: "Tailored solutions for large organizations",
  //       icon: () => (
  //         <svg
  //           xmlns="http://www.w3.org/2000/svg"
  //           width="24"
  //           height="24"
  //           viewBox="0 0 24 24"
  //         >
  //           <path
  //             fill="none"
  //             stroke="currentColor"
  //             stroke-linecap="round"
  //             stroke-linejoin="round"
  //             stroke-width="1.5"
  //             d="M21 11.183V8.28c0-1.64 0-2.46-.404-2.995s-1.318-.794-3.145-1.314a25 25 0 0 1-3.229-1.173C13.023 2.266 12.424 2 12 2s-1.023.266-2.222.798c-.88.39-1.98.818-3.229 1.173c-1.827.52-2.74.78-3.145 1.314C3 5.82 3 6.64 3 8.28v2.903c0 5.625 5.063 9 7.594 10.336c.607.32.91.481 1.406.481s.799-.16 1.406-.48C15.937 20.182 21 16.807 21 11.182"
  //             color="currentColor"
  //           />
  //         </svg>
  //       ),
  //     },
  //   ],
  // },
  // {
  //   name: "Developers",
  //   sublinks: [
  //     {
  //       name: "Community Docs",
  //       href: "https://docs.permguard.com/community",
  //       description: "Guides and tools to support community devs",
  //       icon: BookOpenIcon,
  //     },
  //     {
  //       name: "Enterprise Docs",
  //       href: "http://docs.permguard.com/enterprise",
  //       description: "Advanced documentation for enterprise needs",
  //       icon: BookOpenIcon,
  //     },
  //   ],
  // },
  // {
  //   name: "Support & Pricing",
  //   href: "/pricing-support",
  //   description: "Explore support options and pricing plans",
  // },
];

export const Navigation = () => {
  return (
    <nav role="navigation">
      <ul className="gap-4 lg:gap-8 xl:gap-12 hidden md:flex">
        {NAVIGATION.map((el) => (
          <NavLink key={el.name} {...el} />
        ))}
      </ul>
    </nav>
  );
};
